...
if [ "$ostype" = "Darwin" ]; then
    ...
    docker run -d --cpus=1 --memory=2g --restart=always\
      ...
      -e LOG_STREAMING_SERVICE_URL=https://stress.harness.io/log-service/ \
      -e DELEGATE_TAGS="macos-$archtype" \
      -e RUNNER_URL=http://host.docker.internal:3000 \
      -e MANAGER_HOST_AND_PORT=https://stress.harness.io harness/delegate:$DELEGATE_IMAGE_VERSION
